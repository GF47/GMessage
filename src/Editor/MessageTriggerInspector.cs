...
namespace GMessage.Editor.Inspectors
{
    ...
    public class MessageTriggerInspector : UnityEditor.Editor
    {
        ...
        private void OnEnable()
        {
            ...
            _moduleNameArray = new string[fields.Length + 1];

            _moduleNameArray[0] = "Global";
            for (int i = 0; i < fields.Length; i++)
            {
                _moduleIDArray[i] = (int)fields[i].GetRawConstantValue();
                _moduleNameArray[i + 1] = $"[ {fields[i].Name} ] {fields[i].GetCustomAttribute<DescriptionAttribute>()?.Description}";
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            flag = EditorGUILayout.Popup("Modules", flag + 1, _moduleNameArray);
            trigger.moduleID = flag != 0 ? _moduleIDArray[flag - 1] : Dispatcher.GLOBAL;
            ...
            for (int i = 0; i < trigger.Triggers.Count; i++)
            {
                ...
                msg.id = flag != 0 ? _messageIDArray[flag - 1] : 0;
